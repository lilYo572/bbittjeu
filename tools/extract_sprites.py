...
# Hauteur en pixels de chaque ennemi dans le jeu. Le Lourd et le Volant sont
# plus imposants, le Courreur est un peu plus petit et nerveux.
HAUTEURS = {
    "Serra": 1.00,
    "Serra-Boost": 1.00,
    "Serra-Lanceur": 1.05,
...
def main() -> int:
    ...
    for fichier in sorted(os.listdir(source)):
        ...
        if nom not in HAUTEURS:
            continue
        h = round(base * HAUTEURS[nom])
        img = extraire(os.path.join(source, fichier), h)
        dest = os.path.join(sortie, f"{nom}.png")
        img.save(dest)
        manifeste[nom] = {"fichier": f"{nom}.png", "w": img.width, "h": img.height}
        print(f"{nom:16s} -> {img.width:3d}x{img.height:3d} px")
